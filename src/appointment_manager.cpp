...
Appointment::Appointment(std::string client_name, std::string offer_name,
    ...
    : client_name{std::move(client_name)}, offer_name{std::move(offer_name)}, date{date}, time{time} {
}
...
Appointment::parseDate(const std::string &date_str) {
  ...
  return std::chrono::year_month_day{std::chrono::year{t.tm_year + 1900},
...
std::vector<std::string> Appointment::serialize(const Appointment &entry) {
  std::chrono::hh_mm_ss<std::chrono::minutes> time_display(entry.time); // Renamed variable
  ...
  // For wider compatibility, let's stick to ostringstream for formatting date/time to string here
  std::ostringstream date_ss;
  date_ss << std::setfill('0') << std::setw(2) << static_cast<unsigned>(entry.date.day()) << '/'
          << std::setfill('0') << std::setw(2) << static_cast<unsigned>(entry.date.month()) << '/'
...
AppointmentManager::getAvailableSlots(const std::chrono::year_month_day &date,
  ...
  for (const auto &appointment : getEntries()) {
    if (appointment.date == date) {
      ...
      auto offer_it = std::find_if(offer_entries.begin(), offer_entries.end(), // Renamed to offer_it
      ...
      booked_slots.emplace_back(
          appointment.time, appointment.time + offer_it->duration); // Use offer_it
    }
  }
  ...
  std::vector<std::pair<std::chrono::minutes, std::chrono::minutes>> merged_booked_slots;
  if (!booked_slots.empty()) {
    merged_booked_slots.push_back(booked_slots.front());
    for (size_t i = 1; i < booked_slots.size(); ++i) {
      if (booked_slots[i].first < merged_booked_slots.back().second) { // Overlap or adjacent
        merged_booked_slots.back().second = std::max(merged_booked_slots.back().second, booked_slots[i].second);
      } else {
  ...
  // Filter out slots that are outside working hours (e.g. if all slots are booked or day ends early)
  // This step might be redundant if current_time logic is perfect but good for safety.
   available_slots.erase(
       std::remove_if(available_slots.begin(), available_slots.end(),
                      [&](const AppointmentSlot& slot) {
                        return slot.start_time >= slot.end_time || // Invalid slot
                               slot.end_time <= start_working_day ||   // Slot ends before start
                               slot.start_time >= end_working_day;    // Slot starts after end
                      }),
       available_slots.end());
...
bool AppointmentManager::canScheduleOffer(
  ...
  if (slots.empty() && (day_starts_at < day_ends_at) ) { // if slots is empty but it's a valid working day
     // this condition means the whole day is available if no booked appointments were found by getAvailableSlots
     // however, getAvailableSlots should return the [09:00, 17:00] slot if empty
     // if slots is truly empty, it means no availability.
     // Let's rely on getAvailableSlots. If it's empty, it means no slot.
  }


  for (const auto &slot : slots) {
    if (slot.start_time <= proposed_start_time && slot.end_time >= proposed_end_time) {
      return true;
    }
  }
